...
export class StocktableComponent implements OnInit {
  ...
  stockData: MatTableDataSource<AssetTableDTO> = new MatTableDataSource<AssetTableDTO>([]);
  displayedColumns: string[] = [
    'aktienFondsName', 'menge', 'eintiegkurs', 'einzelwert', 'gesamtwert',
    'rendite', 'rendite_in_procent', 'buy', 'sell'
  ];
  ...
  async loadStockInfo() {
    try {
      const data: AssetTableDTO[] = await firstValueFrom(this.portService.getStockPortfolio());
      this.stockData.data = data;
      this.featureFlagService.log(3, 'Stock Data loaded successfully');
  ...
  async initializeCharts(){
    ...
    const chartData = this.stockData.data.map(item => ({
  ...
  setLabelValues(gesamtwert: number, rendit: number) {
    ...
    if (gesamtwertLabel) {
      this.renderer.setProperty(gesamtwertLabel, 'textContent', gesamtwert ? gesamtwert.toFixed(2) : '0.00');
    }
    if (renditLabel) {
      this.renderer.setProperty(renditLabel, 'textContent', rendit ? rendit.toFixed(2) : '0.00');
      this.renderer.setStyle(renditLabel, 'color', rendit >= 0 ? 'green' : 'red');
    }
  }
  ...
  async loadRendite() {
    try {
      const data: any = await firstValueFrom(this.portService.getRenditeStock());
      if (data) {
  ...
  async openBuyDialog(stock: AssetTableDTO) {
    try {
      ...
      this.featureFlagService.log(1, `Error opening buy dialog: JSON.stringify(data)`);
      this.dataErrorDialog();
    }
  }
  ...
  async openSellDialog(stock: AssetTableDTO) {
    try {
      ...
      this.featureFlagService.log(1, `Error opening sell dialog: JSON.stringify(data)`);
      this.dataErrorDialog();
    }
  }
